...
parser.add_argument('--img_path', default=r"/data/yuzun/MRI_data/hcp/hcp_image/A/val/119025_135.png", help='image path')
parser.add_argument(
    '--model_path', default=r"/data/yuzun/AESRGAN_model/single/", help='single model list path')
...
for i in range(5000, 400000, 5000):
    path = modelpath + r"/net_d_" + str(i) + ".pth"
    l = torch.load(path)  # 加载的D模型

    p = uNet.state_dict()  # 程序中定义的网络模型
    n = {}
    n["params"] = {}
    for k in l["params"]:
        k1 = k.replace("layer0.", "")
